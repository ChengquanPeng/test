...
//	}
    ...
    private synchronized void runByJobv2(int t) {
        ...
        for (StockBaseInfo s : codelist) {
            try {
                // 监听池
                code = s.getCode();
                this.processingByCode(s, getPool(code, poolMap, poolList), listLast, histMap, sbc, xddz, xdjc, yjm1, yjm2);
            } catch (Exception e) {
    ...
    public synchronized void runByCode(String code, int t) {
        ...
        List<CodeBaseModel2> listLast = new LinkedList<CodeBaseModel2>();
        this.processingByCode(s, pool, listLast, histMap, new StringBuffer(), new StringBuffer(), new StringBuffer(), new StringBuffer(), new StringBuffer());
//		if (listLast.size() > 0) {
//			codeBaseModel2Dao.saveAll(listLast);
//		}
//		monitorPoolDao.save(pool);
    ...
    private void processingByCode(StockBaseInfo s, MonitorPoolTemp pool, List<CodeBaseModel2> listLast, Map<String, CodeBaseModel2> histMap, StringBuffer sbc, StringBuffer xddz, StringBuffer xdjc, StringBuffer yjm1, StringBuffer yjm2) {
...
//			}
        ...
        if (s.getCompnayType() == 1 && newOne.getZfStatus() == 1 && newOne.getPb() > 0 && newOne.getPb() < 1 && newOne.getZfjjup() >= 1) {
            isOk2 = true;
        }
            ...
            if (newOne.getFinOK() > 0 || newOne.getBousOK() > 0) {
                ...
                if (newOne.getHolderNumP5() > 40.0) {// 三大股东持股比例
                    if (newOne.getHolderNumP5() > 45.0
                        ...
                        if (newOne.getZfStatus() == ZfStatus.DONE.getCode() && newOne.getZfself() == 1 && newOne.getZfObjType() != 3) {
                            isOk8 = true;
        ...
        // 底部大宗
        if (db2 && (newOne.getDzjyp365d() >= Constant.DZ_RATE || newOne.getDzjy365d() >= Constant.DZ_WARNING_LINE_YEAR)) {
            isOk1 = true;
        }
        // 标准小票-大宗
        ...
        // 标准小票-减持
        if (newOne.getPls() != 2 && isOk7 && rhss.getLastPlanDate() > 0 && rhss.getLastPlanDate() >= newOne.getReduceLastPlanDate()) {// 大于等于是保持6662=1
            newOne.setShooting6662(1);
        } else {
            newOne.setShooting6662(0);
        }
        // 底部小票模式或者人工,减持计划通知
        if (newOne.getPls() != 2 && (isOk7 || newOne.getPls() == 1) && rhss.getLastPlanDate() > 0 && rhss.getLastPlanDate() > newOne.getReduceLastPlanDate()) {//// 没有等于为了通知
...
//				pool.setRemark(Constant.AUTO_MONITOR + TagUtil.getSystemPoint(newOne, Constant.FEN_HAO));
    ...
    private void baseAnalyse(StockBaseInfo s, CodeBaseModel2 newOne, DaliyBasicInfo2 lastTrade, StringBuffer yjm1, StringBuffer yjm2) {
        String code = s.getCode();
        log.info("Code Model  processing for code:{}", code);
        // 基本面池
        // 财务
        List<FinanceBaseInfo> fbis = financeService.getFinacesReportByLteDate(code, tradeDate, EsQueryPageUtil.queryPage8);
        if ((fbis == null || fbis.size() < EsQueryPageUtil.queryPage8.getPageSize())) {
            if (stockBasicService.onlinePreYearChk(code, pre2Year)) {
                ErrorLogFileUitl.writeError(new RuntimeException("无最新财务数据,或者少于条数：" + EsQueryPageUtil.queryPage8.getPageSize()), code, tradeDate + "", "Code Model错误,上市超过2年");
            } else {
                ErrorLogFileUitl.writeError(new RuntimeException("无最新财务数据,或者少于条数：" + EsQueryPageUtil.queryPage8.getPageSize()), code, tradeDate + "", "Code Model错误,上市未超过2年");
            }
            return;
        }
        // 基本面-红蓝绿
        ...
        if (chipsZfService.isZfDateOk(zf, pre3Year)) {
            ...
            if (chipsZfService.isZfDateOk(zf, pre3Year)) {
                if (zf.getPrice() > 0 && zf.getPrice() > lastTrade.getClosed()) {
                    newOne.setZfPriceLow(Double.valueOf(CurrencyUitl.cutProfit(lastTrade.getClosed(), zf.getPrice())).intValue());
                }
            }
            // 实际增发类型
...
//		System.err.println("count:" + count);
    ...
    private void color(StockBaseInfo s, CodeBaseModel2 newOne, List<FinanceBaseInfo> fbis, DaliyBasicInfo2 lastTrade, StringBuffer yjm1, StringBuffer yjm2) {
        ...
        if (fa.getCurrYear().getGsjlr() >= CurrencyUitl.YI_N_DOUBLE && fa.getPrevYear() != null) {
            if (fa.getPrevYear().getGsjlr() > 0) {
                double a = CurrencyUitl.roundHalfUp(fa.getCurrYear().getGsjlr() / Double.valueOf(fa.getPrevYear().getGsjlr()));
                if (a >= 2) {
                    newOne.setFinDbl(1);
                }
            }
            if (fa.getCurrJidu().getGsjlr() > 0) {
                double a = CurrencyUitl.roundHalfUp(fa.getCurrJidu().getGsjlr() / Double.valueOf(fa.getPrevYear().getGsjlr()));
                if (a >= 2) {
                    newOne.setFinDbl(1);
                }
            }
        ...
        if (fbis.size() >= 5) {
            ...
            // 应缴税费
            if (f0.getTaxPayable() >= f1.getTaxPayable() && f1.getTaxPayable() >= f2.getTaxPayable() && f2.getTaxPayable() >= f3.getTaxPayable() && f3.getTaxPayable() >= f4.getTaxPayable() && f0.getTaxPayable() > 0) {
                newOne.setBaseRed(1);
                sb1.append(red++).append(".暴雷风险:应交税费连续增长").append(Constant.HTML_LINE);
            }

            // 应收帐连续增长
            if (f0.getAccountrec() >= f1.getAccountrec() && f1.getAccountrec() >= f2.getAccountrec() && f2.getAccountrec() >= f3.getAccountrec() && f3.getAccountrec() >= f4.getAccountrec() && f0.getAccountrec() > 0) {
        ...
        if (fbi.getZcfzl() >= 80) {
            if (fbi.getZcfzl() >= 99) {
                newOne.setBaseRed(1);
                sb1.append(red++).append(".资产负债率超高:").append(CurrencyUitl.roundHalfUp(fbi.getZcfzl())).append("%").append(Constant.HTML_LINE);
            } else {
                newOne.setBaseYellow(1);
                sb2.append(yellow++).append(".资产负债率高:").append(CurrencyUitl.roundHalfUp(fbi.getZcfzl())).append("%").append(Constant.HTML_LINE);
            }
        ...
        if (fbi.getKfjlr() > 0 && (fbi.getJyxjlce() < 0 || fbi.getMgjyxjl() < 0)) {
            ...
            if (c >= 4) {
                ...
                if (fa.getCurrYear().getJyxjlce() <= 0 && fa.getCurrYear().getMgjyxjl() <= 0) {
                    ...
                    if (c >= (fbis.size() / 2)) {
                        newOne.setBaseRed(1);
                        sb1.append(red++).append(".暴雷风险:最近").append(c).append("季度经常现金流为负却有扣非净利").append(Constant.HTML_LINE).append("靠融资在运转?").append(Constant.HTML_LINE);
                    }
                }
            }
        }
        ...
        if (fbi.getAccountrecRatio() > 0) {
            if (fbi.getAccountrecRatio() >= 50.0) {
                newOne.setBaseYellow(1);
                sb2.append(yellow++).append(".应收账款超高:").append(fbi.getAccountrecRatio()).append("%").append(Constant.HTML_LINE);
            } else if (fbi.getAccountrecRatio() >= 25.0) {
                newOne.setBaseYellow(1);
                sb2.append(yellow++).append(".应收账款高:").append(fbi.getAccountrecRatio()).append("%").append(Constant.HTML_LINE);
            }
        ...
        if (hy != null) {
            if (hy.getMll() > 0 && hy.getMll() > hy.getMllAvg() && hy.getMllRank() <= 5) {
                newOne.setBaseYellow(1);
                sb2.append(yellow++).append(".毛利率:" + hy.getMll() + " 行业平均:" + hy.getMllAvg() + ", 行业排名:" + hy.getMllRank()).append(Constant.HTML_LINE);
            }
            if (hy.getYszk() > 0 && hy.getYszk() > hy.getYszkAvg() && hy.getYszkRank() <= 5) {
                newOne.setBaseYellow(1);
                sb2.append(yellow++).append(".应收账款:" + hy.getYszk() + " 行业平均:" + hy.getYszkAvg() + ", 行业排名:" + hy.getYszkRank()).append(Constant.HTML_LINE);
            }
            if (hy.getXjl() < 0 && hy.getXjl() < hy.getXjlAvg() && hy.getXjlRank() <= 5) {
                newOne.setBaseYellow(1);
                sb2.append(yellow++).append(".现金流:" + hy.getXjl() + " 行业平均:" + hy.getXjlAvg() + ", 行业排名:" + hy.getXjlRank()).append(Constant.HTML_LINE);
            }
        }
        // 商誉占比
