...
//	}
	...
	private void qx1(int date, CodeBaseModel2 newOne, MonitorPoolTemp pool, StringBuffer qx) {
		...
		boolean dibq1 = dibuqixing(newOne, res);// 2.是否底部旗形(旗形过滤)：1.在底部旗形，2.旗形前没怎么涨,3.是否更优的旗形:进一步判断底部旗形
		if (dibq1) {
			if (newOne.getDibuQixing() == 0) {
	...
	private void qx2(int date, CodeBaseModel2 newOne, MonitorPoolTemp pool, StringBuffer qx) {
		...
		boolean dibq2 = dibuqixing2(newOne, res);// 2.是否底部旗形(旗形过滤)：1.在底部旗形，2.旗形前没怎么涨,3.是否更优的旗形:进一步判断底部旗形
		if (dibq2) {
			if (newOne.getDibuQixing2() == 0) {
				qx.append(stockBasicService.getCodeName2(newOne.getCode())).append(",");
			}
	...
	// CHK Date 之前的验证
	private boolean dibuqixing(CodeBaseModel2 newOne, QiBaoInfo res) {
		if (res == null) {
			return false;
		}
	 ...
	 * CHK Date 之前的验证
	 */
	private boolean dibuqixing2(CodeBaseModel2 newOne, QiBaoInfo res) {
		if (res == null) {
			return false;
		}
		// 底部小票
